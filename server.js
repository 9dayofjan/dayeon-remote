...
const server = http.createServer((req, res) => {
    ...
    if (pathname === '/api/version') {
        ...
        let verData = Object.assign({ version: 720, updatedDate: '2026-08-26 10:25:00', updatedAt: Date.now(), files: ['fastcap.exe', '다연원격_클라이언트.exe', 'agent.js', 'input_ctrl.exe', 'audiocap.exe', 'NAudio.dll', '다연코퍼레이션.exe', 'version.json', 'server_ip.txt', 'node.exe'] }, cachedVersion);
    ...
    if (pathname === '/api/update/file') {
        ...
        const allowedFiles = ['다연원격_클라이언트.exe', '다연원격_관리자.exe', 'agent.js', 'input_ctrl.exe', 'fastcap.exe', 'audiocap.exe', 'NAudio.dll', '다연코퍼레이션.exe', '다연코퍼레이션 관리자.exe', 'version.json', 'server_ip.txt', 'node.exe'];
    ...
    if (pathname === '/api/snapshot') {
        ...
        if (targetPcId && pcSessions[targetPcId]) {
            ...
            if (!buf && p.lastGoodBuffer) {
                buf = p.lastGoodBuffer;
            }
            if (!buf && p.rawBuffers) {
                const keys = Object.keys(p.rawBuffers);
                if (keys.length > 0) buf = p.rawBuffers[keys[0]];
            }
    ...
    function dispatchControlCommand(rawTargetPc, cmdObj) {
        ...
        function pushDirectly(targetId) {
            if (cmdObj.type === 'monitor' || cmdObj.type === 'select_monitor') {
                ...
                if (pcSessions[targetId]) {
                    pcSessions[targetId].activeMonitor = m;
                    pcSessions[targetId].monitor = m;
                }
